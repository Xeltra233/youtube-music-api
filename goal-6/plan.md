...
7. 管理前端提供可实际使用的 YouTube 登录入口；登录完成后写入持久化浏览器档案，并能同步成搜索/下载共用的稳定 jar，最终必须做真实端到端验证。
...
### 3.4 前端 YouTube 登录（追加目标）

用户会从管理前端完成登录，因此“支持已有浏览器档案”只是底层能力，不是最终交付。最终流程必须满足：

1. 仅通过现有管理端认证后才能创建/查看/终止登录会话。
2. 登录使用服务端持久化的专用浏览器档案，不能尝试在普通 iframe 内嵌 Google 登录页。
3. 前端能进入真实交互式登录流程，处理账号选择、密码、二次验证或验证码，并显示明确状态。
4. 登录成功的判据不是“页面跳转成功”，而是档案抽取出的 Cookie 被识别为登录态，并通过搜索与下载验证。
5. 浏览器档案目录可持久化挂载；不同管理员会话不得串用临时控制通道，Cookie/密码不得返回前端日志或 API 响应。
6. T7 先做运行环境与实现路线实证，随后完成后端会话编排、前端 UI、视觉验证和端到端测试；保留 Netscape 文件上传作为回退。
...
| 服务端浏览器登录组件 | 持久化 Chromium 档案、登录会话生命周期与受保护的交互通道 |
| `internal/admin/assets` | YouTube 登录入口、状态、重试/退出与使用说明 |
...
| Google 登录不能被普通 iframe 嵌入 | 使用服务端专用浏览器会话及受保护交互通道，不把跨站登录塞进 iframe |
| 登录流程涉及密码、2FA、验证码 | 只转发交互画面/输入，服务端不记录表单值；会话短时、隔离并可终止 |
| 云容器没有浏览器或持久卷 | T7 实证依赖与部署形态；档案目录必须可持久化，启动时做能力检查 |
...
8. 前端登录完成后，验证“登录态 Cookie → 搜索 → 音频/MP4 下载”完整链路，并进行截图/实际读图的视觉验证。
...
2. 底层同步器先支持已有档案；追加需求要求最终加入服务端专用浏览器和管理前端交互登录，不采用普通 iframe 内嵌 Google 页面。
