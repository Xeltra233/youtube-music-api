...
7. 在“已有外部浏览器档案同步”与“管理前端直接登录 YouTube”两条可选路线中选择更适合长期运行的主路线；两者不是串行前置条件。管理前端路线完成登录后写入服务专用持久化档案，并能同步成搜索/下载共用的稳定 jar，最终必须做真实端到端验证。
...
用户会从管理前端完成登录；用户进一步明确“已有浏览器档案同步”与“前端直接登录”是两条可选路线，而非串行步骤。T7 已完成耐久性比较和实证，结论是：

- **主路线：管理前端直接登录到服务专用的持久化 Chromium 档案。** 该档案只由服务控制，避免日常浏览器锁竞争、外部 profile 路径漂移和桌面用户上下文差异；登录会话结束后继续复用同一档案，并通过 CDP 导出 Cookie 到稳定 jar，不从磁盘外部解密浏览器数据库。
- **可选路线：读取已经登录的外部浏览器档案。** 继续由 `COOKIES_FROM_BROWSER` 支持，适合服务与日常浏览器处于同 OS/用户上下文的本机部署；它独立工作，不是前端登录的前置步骤。
- **文件回退：Netscape 上传。** 两条浏览器路线均失效时仍保留。

主路线的“直接登录”仍通过真实浏览器页面完成，不采用账号密码 API，也不要求先准备另一个浏览器档案。最终流程必须满足：

1. 仅通过现有管理端认证后才能创建/查看/终止登录会话。
2. 登录使用服务端持久化的专用浏览器档案，不能尝试在普通 iframe 内嵌 Google 登录页。
3. 前端能进入真实交互式登录流程，处理账号选择、密码、二次验证或验证码，并显示明确状态。
4. 登录成功的判据不是“页面跳转成功”，而是浏览器通过 CDP 导出的 Cookie 被识别为登录态，并通过搜索与下载验证。
5. 浏览器档案目录可持久化挂载；不同管理员会话不得串用临时控制通道，Cookie/密码不得返回前端日志或 API 响应。
6. T7 先做两条可选路线的运行环境与耐久性实证并确定默认模式，随后完成后端会话编排、前端 UI、视觉验证和端到端测试；保留外部 profile 配置和 Netscape 文件上传作为独立可选来源/回退。

## 4. “是否更稳定”的结论标准

浏览器档案方案预期比手工反复导出文件更省维护，前提是档案本身保持登录且可被当前运行环境解密。它主要改善 Cookie 获取与轮换，不承诺绕过账号退出、风控、出口 IP 或上游协议变化。

T7 的实证进一步区分了两种档案：外部日常浏览器档案仍受 OS/用户/锁/解密条件影响；服务专用 managed profile 由浏览器自身通过 CDP 提供已解密 Cookie，因此作为前端登录默认路线更可控。完整证据和 T8 契约见 `goal-6/t7-login-route.md`。
...
2. 已有外部档案同步与管理前端直接登录是可独立选择的来源；面向前端的默认路线采用服务端专用持久化浏览器档案，不要求先配置外部档案，也不采用普通 iframe 内嵌 Google 页面。
